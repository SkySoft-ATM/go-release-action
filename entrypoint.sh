...
docker build  --build-arg BUILD_VERSION="${BUILD_VERSION}" --build-arg GITHUB_USER="${GITHUB_USER}" --build-arg GITHUB_TOKEN="${GITHUB_TOKEN}" --build-arg APP_NAME="${APP_NAME}" --build-arg APP_DESCRIPTION="${APP_DESCRIPTION}" --build-arg MAIN_FOLDER="${MAIN_FOLDER}" --build-arg PROJECT="${PROJECT}" -t eu.gcr.io/"${PROJECT}"/"${APP_NAME}":"${BUILD_VERSION}" . -f "${DOCKERFILE}"
